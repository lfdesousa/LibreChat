/**
 * The thin HTTP client to the BFF's `/console/presets/*` proxy
 * (`bff/console_presets_proxy.py` / `bff/app.py::console_presets_base|
 * console_presets_proxy` in AuditTrace-AI, consumed here, never modified —
 * mirrors `bff/console_conversations_proxy.py`'s shape byte-for-byte, per
 * the WU-1 precedent).
 *
 * Deliberately mirrors `AuditTraceConversations/client.js::
 * callConsoleConversationsProxy`'s discipline byte-for-byte (same
 * fail-closed contract, same `MissingAccessTokenError`/
 * `SovereignMemoryError` split), reusing that module's `getConfig`/error
 * types rather than duplicating them — the BFF base URL and timeout are
 * the SAME toggle (`AUDITTRACE_BFF_BASE_URL`, `AUDITTRACE_BFF_TIMEOUT_MS`)
 * whether the request lands under `/console/conversations/*` or
 * `/console/presets/*`; only the path prefix differs.
 *
 * Forwards the CALLER's own access token as-is (`Authorization: Bearer
 * <token>`) — the BFF does the RFC 8693 exchange for the
 * `memory:presets:{read-own,write}` scopes server-side; this fork never
 ...
 * @param {string} [params.path] - path suffix after `/console/presets`,
 *   e.g. `""` (base — list/create) or `"<presetId>"`. An empty/undefined
 *   suffix hits the base path with no trailing slash (mirrors the BFF's
 *   own "path_suffix may be empty" handling).
 ...
 */
async function callConsolePresetsProxy({ method, path, token, query, body }) {
  if (!token) {
    throw new MissingAccessTokenError();
  }

  const { bffBaseUrl, timeoutMs } = getConfig();
  const base = `${bffBaseUrl.replace(/\/+$/, '')}/console/presets`;
  ...
  try {
    ...
    logger.error('[AuditTracePresets] BFF /console/presets unreachable', error);
    throw new SovereignMemoryError(`sovereign presets service unreachable: ${error.message}`, 502);
  }
  ...
  if (response.status >= 400) {
    ...
    throw new SovereignMemoryError(
      `sovereign presets request failed (${response.status}): ${detail}`,
      response.status,
    );
  }
...
module.exports = { callConsolePresetsProxy };
